	...
	<body>
		...
		<div id="show-subject" class="content scaffold-show" role="main">
			...
			<ol class="property-list subject">
				...
				<g:if test="${subjectInstance?.course}">
				<li class="fieldcontain">
					<span id="course-label" class="property-label"><g:message code="subject.course.label" default="Course" /></span>
					
						<span class="property-value" aria-labelledby="course-label"><g:link controller="course" action="show" id="${subjectInstance?.course?.id}">${subjectInstance?.course?.encodeAsHTML()}</g:link></span>
